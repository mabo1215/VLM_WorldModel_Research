...
- Year：2020
## Main Problem:
- 现有时空预测方法在长期预测中效果显著下降，主要面临两个核心挑战：\
(1) 在历史观测中，大量不相关的空间信息将噪声引入到预测中，而这些噪声大多被忽视并导致误差飙升,通过复杂的相关性传播，先前输出的误差在生成长期结果时会干扰即将到来的预测。\
(2) 不相关信息的负面影响，因为当输入的空间大小变大时，错误率会大大增加,并非所有空间位置都包含有助于预测的信息.\
(3):遵循传统的时间序列预测方式，长期预测可以很容易地吸收先前结果中的误差。当每个时空输出将其误差传递给下一个预测时，误差传播比典型的时间序列问题更为显着。
## Core Method:
- 提出了动态切换注意力网络（DSAN），核心思想是通过多头空间注意力（Multi-Space Attention, MSA）显式度量输入与输出之间的时空相关性，从含噪声的大范围输入中动态提取有价值信息，并通过切换注意力机制直接将每个输出与净化后的输入相连，阻断误差传播路径。DSAN包含两个核心框架：(1) 动态注意力编码器（DAE），对全局输入和强相关局部输入分别进行自注意力，通过交叉注意力融合为紧致表示；(2) 切换注意力解码器（SAD），通过双解码器结构将每个未来时间步的输出直接连接到不同注意力子空间中的输入，实现误差隔离。
## Model Architecture:
- DAE（Dynamic Attention Encoder）：\
  Enc-G（全局编码器）：对完整空间输入X进行自注意力+STPE（时空位置编码）→ 全局特征\
  Enc-D（局部编码器）：对强相关局部子集XD进行自注意力+STPE → 局部特征\
  交叉注意力机制融合全局与局部特征
- SAD（Switch-Attention Decoder）：\
  Dec-S（切换解码器）：在切换注意力子空间中连接输出与DAE净化后的输入\
  Dec-T（时序解码器）：在时序注意力子空间中建模时间依赖\
  双解码器结构实现不同注意力子空间的并行推理
- STPE（Spatial-Temporal Positional Encoding）：为不同空间位置和时间步注入位置信息
- 训练方式：支持联合多步训练（DSAN-LT: 均等权重；DSAN-LTW: 首步80%权重）
## Dataset:
- 1、时空预测任务-人流预测（Crowd Flow Prediction）	Taxi-NYC（纽约出租车数据）、Bike-NYC（纽约共享单车数据）
- 2、时空预测任务-服务利用率预测（Service Utilization Prediction）	CTM（服务时长与请求数数据）
- 每个网格特征：人流预测为 inflow/outflow，服务利用率为 duration（服务时长）/ request（请求数）
- 网格大小：0.1km×0.1km 至 1km×1km，视数据质量而定
## Evaluation Metric:
### 短期预测（Single-step）
- RMSE（均方根误差）
- MAPE（平均绝对百分比误差，%）
...
- 与基线对比的相对提升百分比
## Main Result:
| 任务 | 指标 | DSAN-ST（短期） | STDN | LSTM | MLP | 提升幅度（vs STDN） |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| Taxi-NYC inflow | RMSE | **17.65±0.24** | 19.46±0.21 | 24.04±0.29 | 27.20±0.53 | -9.3% |
| Taxi-NYC outflow | RMSE | **22.86±0.31** | 24.49±0.33 | 29.92±0.32 | 31.93±0.56 | -6.7% |
| Bike-NYC inflow | RMSE | **7.90±0.08** | 8.67±0.10 | 10.53±0.14 | 10.67±0.21 | -8.9% |
| Bike-NYC outflow | RMSE | **8.42±0.09** | 9.43±0.11 | 10.99±0.19 | 11.32±0.21 | -10.7% |
| CTM duration | RMSE | **175.5±45** | 294.3±28 | 241.8±10 | 214.0±22 | -40.4% |
| CTM request | RMSE | **61.75±2.55** | 66.38±1.53 | 61.63±2.26 | 57.15±3.87 | -7.0% |
| Taxi-NYC 12步平均 | RMSE降低 | **至少5%** | — | — | — | 优于所有基线 |
## Limitation:
- 在CTM服务利用率预测任务上，深度学习方法相对传统方法的优势不如在人流预测中明显，说明该方法对弱相关、高噪声数据的鲁棒性仍有提升空间
- 注意力机制的感受受限于输入空间的网格划分方式，对极不规则的空间分布可能不够灵活
## Relevance to our paper:
- 展示了通过显式的多头注意力机制（MSA）来建模动态时空相关性的技术路线，对于设计长时序世界模型中的状态预测模块具有重要的方法论启示。
# review 16
## Paper Title:
...
- Year：2019
## Main Problem:
- 现有的工作对空间依赖性和时间动态做出了强有力的假设，即空间依赖性在时间上是静止的，而时间动态是严格周期性的。然而，实际上，空间依赖性可能是动态的，并且时间动态可能在一个时期到另一个时期之间存在一些扰动。在本文中，我们做出了两个重要的观察：\
（1）位置之间的空间依赖性是动态的；\
 （2）时间依赖性遵循每日和每周的模式，但由于其动态的时间变化而不是严格周期性的。
## Core Method:
- 提出了时空动态网络（STDN），首次在一个统一框架中同时解决空间依赖动态性和时间模式偏移两个问题。核心创新包括：\
(1) 流量门控机制（Flow Gating Mechanism）——通过建模区域间的交通流量转移来学习位置间动态相似性，即在每个时间步动态计算位置间相关性而非使用固定的空间邻接矩阵；\
(2) 周期偏移注意力机制（Periodically Shifted Attention Mechanism）——在周/日周期序列上引入可学习的注意力偏移量，自动对齐因时间偏移造成的模式错位，从而更准确地捕获长期周期依赖。
## Model Architecture:
### 1. HA

Historical Average 历史均值法
最简单的基线模型，用历史同期平均值做预测。

### 2. ARIMA

自回归积分移动平均模型，经典传统统计学时间序列模型。

### 3. LR

Linear Regression 线性回归，机器学习基线回归模型。

### 4. MLP

多层感知机，基础全连接神经网络，浅层深度学习模型。

### 5. XGBoost

极端梯度提升树，主流集成学习树模型，表格 / 时序预测强力机器学习模型。

### 6. LinUOTD

针对时空时序的线性时空预测模型（多用于城市人流、路网流量预测）。

### 7. ConvLSTM

卷积长短期记忆网络，**时空预测标杆模型**，CNN 提取空间特征 + LSTM 捕捉时间依赖，常用于交通、降水预测。

### 8. DeepSD

深度时空分解网络，Deep Spatial Decomposition，时空序列深度分解预测模型。

### 9. ST-ResNet

Spatio-Temporal Residual Network 时空残差网络，城市人流量预测经典深度学习模型，残差结构堆叠捕捉远近时空依赖。

### 10. DMVST-Net

Deep Multi-View Spatio-Temporal Network 多视图深度时空网络，融合多视角时空特征的进阶时空预测神经网络。
## Dataset:
- 1、NYC-Taxi：纽约市2015年22,349,490条出租车行程记录（1/1-3/1，前40天训练，后20天测试）
- 2、NYC-Bike：纽约Citi Bike系统2016年2,605,648条共享单车行程记录（7/1-8/29，前40天训练，后20天测试）
- 数据预处理：Min-Max归一化至[0,1]，滑动窗口采样，过滤低流量样本（<10）
## Evaluation Metric:
- RMSE（均方根误差）
- MAPE（平均绝对百分比误差，%）
- 学生t检验验证显著性
- 对比基线：HA、ARIMA、Ridge、LinUOTD、XGBoost、MLP、ConvLSTM、DeepSD、ST-ResNet、DMVST-Net
## Main Result:
| 数据集 | 指标 | STDN | 最佳传统基线 | 最佳神经网络基线 | 关键发现 |
| :--- | :--- | :---: | :---: | :---: | :--- |
| NYC-Taxi | RMSE | **最低** | XGBoost（次优） | DMVST-Net（次优） | 显著优于所有基线（p<0.01) |
| NYC-Taxi | MAPE | **最低** | XGBoost（次优） | DMVST-Net（次优） | 传统时序方法（HA/ARIMA）表现最差 |
| NYC-Bike | RMSE | **最低** | XGBoost（次优） | DMVST-Net（次优） | 回归方法优于纯时序方法 |
| NYC-Bike | MAPE | **最低** | XGBoost（次优） | DMVST-Net（次优） | 空间信息显著提升预测精度 |
| 消融-流量门控 | RMSE | LSTN-FGM > LSTN-FI > LSTN | — | — | 流量门控机制比简单拼接流量特征更有效 |
| 消融-周期偏移注意力 | RMSE | STDN (完整) > LSTN-PSAM > LSTN | — | — | 周期偏移注意力提升长期周期依赖建模 |
## Limitation:
- 周期偏移注意力为每个位置学习独立的偏移量，在区域数量大时参数量增长较快
- 实验仅在30分钟时间粒度下进行，在更细粒度（如5分钟）或更粗粒度（如2小时）设置下的表现未知
## Relevance to our paper:
- 作为时空预测领域的重要奠基工作，首次明确指出现有方法中"静态空间依赖"和"严格周期性"假设的局限性，为动态时空建模提供了理论依据。，世界模型在建模物理规律时必须考虑这些非平稳特性。
